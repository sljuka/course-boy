...
function checkRefsAndKeywords$1(it) {
  ...
  if (schema.$ref && opts.ignoreKeywordsWithRef && (0, util_1$S.schemaHasRulesButRef)(schema, self.RULES)) {
...
function validateAdditionalItems$1(cxt, items2) {
  ...
  function validateItems(valid2) {
    gen.forRange("i", items2.length, len, (i) => {
      cxt.subschema({ keyword: keyword2, dataProp: i, dataPropType: util_1$J.Type.Num }, valid2);
      if (!it.allErrors)
  ...
  code(cxt) {
    ...
    function validateItems(valid2, from) {
      gen.forRange("i", from, len, (i) => {
        cxt.subschema({ keyword: "unevaluatedItems", dataProp: i, dataPropType: util_1$t.Type.Num }, valid2);
        if (!it.allErrors)
...
const allowedIconExtensions = /* @__PURE__ */ new Set([".png", ".jpg", ".jpeg", ".webp", ".svg"]);
const iconMimeTypes = {
  ".jpeg": "image/jpeg",
  ".jpg": "image/jpeg",
  ".png": "image/png",
...
function isCourseManifest(value) {
  if (!value || typeof value !== "object") {
    return false;
  }
  const manifest = value;
  return typeof manifest.id === "string" && typeof manifest.version === "string" && typeof manifest.courseType === "string" && typeof manifest.sharedIndex === "string" && typeof manifest.sharedPath === "string" && typeof manifest.localesPath === "string" && isLocale(manifest.defaultLocale) && Array.isArray(manifest.supportedLocales) && manifest.supportedLocales.every(isLocale);
}
function isSharedCourseIndex(value) {
  if (!value || typeof value !== "object") {
    return false;
  }
...
async function listCourseRecords(rootDirectoryPath) {
  let directoryEntries;
  try {
    directoryEntries = await fs$1.readdir(rootDirectoryPath, {
      withFileTypes: true
    });
  } catch (error2) {
  ...
  const courseRecords = await Promise.all(
    directoryEntries.filter((directoryEntry) => directoryEntry.isDirectory()).map(async (directoryEntry) => {
...
function resolveLocalizedCourseMetadataPath(courseRecord, locale) {
  return path.join(
...
async function readLocalizedCourseMetadata(courseRecord, preferredLocale) {
...
async function readSharedCourseIndex(courseRecord) {
  return readJsonFile(
    path.join(courseRecord.directoryPath, courseRecord.manifest.sharedIndex),
    isSharedCourseIndex
  );
}
async function readSharedLessonDefinition(courseRecord, lessonId) {
  return readJsonFile(
    path.join(
      courseRecord.directoryPath,
...
async function resolveLessonIconUrl(courseRecord, relativeIconPath) {
  if (!relativeIconPath) {
    return null;
  }
  ...
  if (relativeToCourseRoot.startsWith("..") || path.isAbsolute(relativeToCourseRoot) || !allowedIconExtensions.has(path.extname(resolvedIconPath).toLowerCase())) {
    return null;
  }
  try {
    ...
    const fileContents = await fs$1.readFile(resolvedIconPath);
    return `data:${mimeType};base64,${fileContents.toString("base64")}`;
  } catch {
    return null;
  }
}
async function readLessonPreviews(courseRecord, lessonIds, preferredLocale) {
  return Promise.all(
    lessonIds.slice(0, 6).map(
      (lessonId) => readLessonPreview(courseRecord, lessonId, preferredLocale)
    )
  );
}
...
async function listCourses(rootDirectoryPath, preferredLocale) {
...
async function getCourseDetails(rootDirectoryPath, courseId, preferredLocale) {
  const courseRecords = await listCourseRecords(rootDirectoryPath);
  const courseRecord = courseRecords.find((record) => record.manifest.id === courseId) ?? null;
  if (!courseRecord) {
    return null;
  }
  const [localizedCourseMetadata, sharedCourseIndex] = await Promise.all([
...
ipcMain$1.handle("courses:list", (_event, locale) => {
  return listCourses(getLocalCoursesRoot(), locale);
});
ipcMain$1.handle(
  "courses:get",
  (_event, courseId, locale) => {
